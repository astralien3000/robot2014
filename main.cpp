...
void asserv_init(void) {
  ...
  // Robot
  pid_a.setGains(3000, 0, 0);
  pid_a.setMaxIntegral(10000);
  pid_a.setOutShift(6);
  
  pid_d.setGains(3000, 0, 0);
...
int main(int argc, char* argv[]) {
  ...
  //ziegler_nichols_algo(motc_l, enc_conv_l, pid_l);
  //return 0;
  ...
  s32 last_l = ENC_L, last_r = ENC_R;
  while(Aversive::isRunning()) {
    s32 t1 = (s32)FPGA_MS << 16 + FPGA_US, t2 = t1;

    if((odo.getValue().coord(1) - cmd.coord(1))*(odo.getValue().coord(1) - cmd.coord(1)) < 10 && (odo.getValue().coord(0) - cmd.coord(0))*(odo.getValue().coord(0) - cmd.coord(0)) < 5 && (last_l - ENC_L)*(last_l - ENC_L) < 2 & (last_r - ENC_R)*(last_r - ENC_R) < 2) {
